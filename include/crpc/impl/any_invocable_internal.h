...
//
// Implementation details for `std::AnyInvocable`
...
// IWYU pragma: private, include "std/functional/any_invocable.h"
...
// depend on the cv/ref qualifiers of the function type.
            ...
            std::enable_if_t<target_type == TargetType::kPointer, int> = 0>
            void Initialize(F&& f) {
...
#pragma GCC diagnostic pop
            ...
            template <TargetType target_type, class QualDecayedTRef, class F,
            typename = std::enable_if_t<target_type == TargetType::kOther>>
            void Initialize(F&& f) {
                InitializeStorage<QualDecayedTRef>(std::forward<F>(f));
            }
...
// std::base_internal::is_invocable_r because we can't rely on it to give the
